...
test('Directory renders with correct number of children', async () => {
  ...
  expect(tree.children[1].children.length).toEqual(50)
  //child nodes should be contained in divs
  expect(tree.children[1].children[0].type).toBe('div')
  //they should be full width collumns
  expect(tree.children[1].children[12].props.className).toBe('col-md-12')
  //the second child of the child nodes should have a child containing text
  expect(typeof tree.children[1].children[12].children[0].children[1].children[0]).toBe('string')
